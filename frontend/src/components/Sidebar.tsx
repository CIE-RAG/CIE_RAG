...
import { Plus, MessageSquare, X, Clock, ChevronLeft, ChevronRight, Trash2 } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const handleDelete = async (conversation: Conversation) => {
    ...
    try {
      const response = await axios.delete(`http://localhost:8000/session/${conversation.session_id}`);
    ...
    <div className={`${isCollapsed ? 'w-16' : 'w-80'} bg-white h-full border-r border-[#313c71]/20 flex flex-col shadow-xl transition-all duration-300`}>
      <div className="p-4 border-b border-[#313C71]/20">
        ...
        <button
          ...
          className={`w-full bg-[#EF7F1A] text-white py-3 rounded-xl font-semibold hover:bg-[#E75728] active:bg-[#E75728] transition-all duration-300 transform hover:scale-[1.02] active:scale-[0.98] shadow-lg hover:shadow-xl flex items-center justify-center space-x-2 focus:outline-none focus:ring-4 focus:ring-[#67753A]/20 ${isCollapsed ? 'px-2' : 'px-4'}`}
            ...
            {conversations.map((conversation) => (
              <div key={conversation.id} className="flex items-start space-x-2">
                <button
                  ...
                  className={`flex-1 text-left p-3 rounded-xl transition-all duration-300 ${activeConversation === conversation.id ? 'bg-[#313C71]/30 backdrop-blur-sm text-[#313C71] shadow-lg transform scale-[1.02]' : 'bg-[#313C71]/10 backdrop-blur-sm hover:bg-[#313C71]/20 text-[#313C71]/90 hover:shadow-md'} group ${isCollapsed ? 'px-2' : 'px-3'}`}
                  title={isCollapsed ? conversation.title : ''}
                >
                  <div className="flex items-start space-x-3">
                    <div className={`w-2 h-2 rounded-full mt-2 flex-shrink-0 ${activeConversation === conversation.id ? 'bg-[#313C71]' : 'bg-[#313C71]/60'}`} />
                    {!isCollapsed && (
                      <div className="flex-1 min-w-0">
                        <h3 className="font-semibold text-sm truncate mb-1">{conversation.title}</h3>
                        <div className="flex items-center justify-between">
                          <p className={`text-xs ${activeConversation === conversation.id ? 'text-[#313C71]/80' : 'text-[#313C71]/60'}`}>
                            {conversation.messages.length} messages
                          </p>
                          {conversation.messages.length > 0 && (
                            <div className={`flex items-center space-x-1 text-xs ${activeConversation === conversation.id ? 'text-[#313C71]/80' : 'text-[#313C71]/50'}`}>
